-- =====================================================
-- FootMatch V4 — DEPLOY VERSION (safe for Supabase)
-- No DROP statements — clean install only
-- =====================================================
...
CREATE POLICY "operators_insert_own" ON operators FOR INSERT WITH CHECK (auth.uid() = profile_id);
...
CREATE POLICY "applications_select" ON operator_applications FOR SELECT
  USING (auth.uid() = profile_id OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
CREATE POLICY "applications_insert_own" ON operator_applications FOR INSERT WITH CHECK (auth.uid() = profile_id);
...
CREATE POLICY "registrations_update_own" ON match_registrations FOR UPDATE USING (auth.uid() = player_id);
CREATE POLICY "registrations_delete_own" ON match_registrations FOR DELETE USING (auth.uid() = player_id);
...
CREATE POLICY "messages_delete_own" ON match_messages FOR DELETE USING (auth.uid() = sender_id);
